...
public class ZNetApiAtExample {
	...
	public ZNetApiAtExample() throws XBeeException {
		try {
			...
			System.out.println("SH is " + ByteUtils.toBase16(((AtCommandResponse)xbee.sendAtCommand(new AtCommand("SH"))).getValue()));
			System.out.println("SL is " + ByteUtils.toBase16(((AtCommandResponse)xbee.sendAtCommand(new AtCommand("SL"))).getValue()));
...
//			this.configureEndDevice(xbee);
	...
	private void configureEndDevice(XBee xbee) throws XBeeException {
		...
		System.out.println("RE is " + response);
	...
	private void configureIOSamples(XBee xbee) throws XBeeException {			
		...
		System.out.println("IR is " + response);

		// set pin 20 to monitor digital input
		response = xbee.sendAtCommand(new AtCommand("DO", 0x3));
		System.out.println("DO is " + response);

		// set pin 19 to monitor anaSystem.out.println input
		response = xbee.sendAtCommand(new AtCommand("D1", 0x2));
		System.out.println("D1 is " + response);
		
		// set pin 18 to monitor anaSystem.out.println input
		response = xbee.sendAtCommand(new AtCommand("D2", 0x2));
		System.out.println("D2 is " + response);
		
		// set pin 16 to monitor digital input
		response = xbee.sendAtCommand(new AtCommand("D6", 0x3));
		System.out.println("D6 is " + response);
